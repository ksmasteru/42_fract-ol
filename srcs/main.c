...
void ft_put_fractal(int ac, char **av, t_draw *data)
{
  ...
  if (data->is_julia == 0)
  {
    data->c_real = d_atoi((data->av)[2]); /* calucalte this one time*/
    data->c_img = d_atoi((data->av)[3]); /* if there is an error with arg should quit here*/
...
int main(int ac, char **av) {
    ...
    if (ac == 2  || ac == 4)
    {
      if (ac == 4 &&  ft_strcmp("julia", av[1]) == 0)
        mlx_data.is_julia = 0;
      else if (ac == 2 && ft_strcmp("mandelbrot", av[1]) == 0)
